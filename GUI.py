...
class FlugzeugDatenApp(ctk.CTk):
    def __init__(self):
        ...
        for nation in self.nationen:
            self.flugzeuge_pro_nation[nation] = sorted(list(set(zeile['Flugzeug'] for zeile in flugzeugdaten if zeile['Nation'] == nation)))
    ...
    def delete_entry(self):
        if self.selected_nation and self.selected_flugzeug:
            ...
            flugzeugdaten[:] = [zeile for zeile in flugzeugdaten if not (zeile['Nation'] == self.selected_nation and zeile['Flugzeug'] == self.selected_flugzeug)]
            self.flugzeuge_pro_nation[self.selected_nation].remove(self.selected_flugzeug)
            self.text_area.insert("1.0", f"Flugzeug '{self.selected_flugzeug}' aus '{self.selected_nation}' gelöscht.\n")
            self.save_to_csv()  # Speichere die Änderungen
            self.flugzeuge_combobox.configure(values=self.flugzeuge_pro_nation[self.selected_nation])  # Aktualisiere die Combobox
        else:
    ...
    def save_to_csv(self):
        try:
            with open(dateipfad, 'w', newline='', encoding='utf-8') as datei:
                fieldnames = ['Nation', 'Flugzeug', 'BattleRating', 'Klasse', 'Turnrate', 'Steigrate', 'Geschwindigkeit', 'beiHoehe', 'maxHoehe']
